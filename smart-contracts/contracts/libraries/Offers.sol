...
library Offers {
     ...
     * 
     */
    function set(OfferSet storage offers, Offer memory offer) internal {
        require(offer.id > 0, "Set: Invalid offer ID");

        uint256 offerPos = offers._positions[offer.id];
        require(offerPos > 0, "Set: Unexisting offer ID");

        offers._values[_positionToIndex(offerPos)] = offer;
    }
    ...
    function remove(OfferSet storage offers, uint32 offerId)
        internal
    {
        require(offerId > 0, "Remove: Invalid offer ID");
    ...
    function contains(OfferSet storage offers, uint32 offerId)
        ...
        returns (bool)
    {
        require(offerId > 0, "Contains: Invalid offer ID");
    ...
    {
        require(offerId > 0, "Get: Invalid offer ID");

        uint256 offerPos = offers._positions[offerId];
        require(offerPos > 0, "Get: Unexisting offer ID");
